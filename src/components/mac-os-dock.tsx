...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const getResponsiveConfig = useCallback(() => {
    ...
    if (smallerDimension < 480) {
      // Mobile phones
      return {
        baseIconSize: Math.max(30, smallerDimension * 0.075),
        maxScale: 1.25,
  ...
  const padding = Math.max(8, baseIconSize * 0.12);
    ...
    <div 
      ...
      style={{
        ...
        boxShadow: `
          0 ${Math.max(10, baseIconSize * 0.2)}px ${Math.max(40, baseIconSize * 0.8)}px rgba(0, 0, 0, 0.4),
        ...
        {apps.map((app, index) => {
          ...
          return (
            <div
              ...
              className="absolute cursor-pointer flex flex-col items-center justify-end group/icon"
              ...
              <AnimatePresence>
                {hoveredIndex === index && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: -baseIconSize * scale - 20, x: '-50%', scale: 1 }}
                    exit={{ opacity: 0, y: 10, x: '-50%', scale: 0.8 }}
                    className="absolute left-1/2 px-3 py-1.5 rounded-lg bg-[#1a1a1a]/90 backdrop-blur-md border border-white/10 text-white text-[10px] font-medium tracking-wide whitespace-nowrap shadow-2xl pointer-events-none z-[100]"
                  >
                    {app.name}
                    <div className="absolute bottom-[-4px] left-1/2 -translate-x-1/2 w-2 h-2 bg-[#1a1a1a]/90 border-r border-b border-white/10 rotate-45" />
                  </motion.div>
                )}
              </AnimatePresence>
              <img
