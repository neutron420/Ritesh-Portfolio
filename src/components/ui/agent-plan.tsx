...
export default function Plan({ initialTasks: propTasks }: { initialTasks?: Task[] }) {
  const [tasks, setTasks] = useState<Task[]>(propTasks || []);
  const [expandedTasks, setExpandedTasks] = useState<string[]>(
    propTasks ? propTasks.map(t => t.id) : ["1"]
  );

  React.useEffect(() => {
    if (propTasks) {
      setTasks(propTasks);
      setExpandedTasks(propTasks.map(t => t.id));
    }
  }, [propTasks]);
  ...
  return (
    <div className="w-full text-foreground">
      <motion.div 
        className="bg-card/50 backdrop-blur-sm border-border/40 rounded-lg border overflow-hidden"
